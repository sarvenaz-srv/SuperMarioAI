...
class Game:
    ...
    def choice(self, actionsList, scores, num):
        ...
        choicesScores = []
        for i in range(num):
            choice = self.get_choice(actionsList, scores)
            choices.append(actionsList[choice])
            choicesScores.append(scores[choice])
        return choices, choicesScores
    ...
    def genetic_algorithm(self):
        ...
        parentsList, parentsScores = self.choice(initialPopulation, initialScores, 250)
        count = 0
        ...
        while True:
            if count == 10:
                break
            # baztarkibi
            children = self.jump(self.combine(parentsList, parentsScores, 250))
